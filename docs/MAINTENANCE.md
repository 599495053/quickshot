...
- Keep the packaged `privacy-ocr-fallback` and `overlay-edit-smoke` self-tests in `release.ps1 -SmokeTest`; add `-PrivacySelfTest` and `-OverlaySelfTest` to installer verification runs for builds that include the hidden self-test entry point.
