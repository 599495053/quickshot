...
class BaseCardDelegate(QStyledItemDelegate):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._font_title = QFont("Microsoft YaHei", 9, QFont.Weight.DemiBold)
    ...
    def _draw_card_and_thumb(self, painter: QPainter, rect, is_selected: bool, is_hover: bool, thumb_pixmap, placeholder: str = "加载中..."):
        """绘制卡片背景和缩略图区域，返回 (text_x, text_w) 供子类绘制文本。"""
        if is_selected:
        ...
        if isinstance(thumb_pixmap, QPixmap) and not thumb_pixmap.isNull():
            ...
            painter.drawText(thumb_rect, Qt.AlignmentFlag.AlignCenter, placeholder)
...
class HistoryItemDelegate(BaseCardDelegate):
    ...
    def paint(self, painter: QPainter, option, index) -> None:
        ...
        text_x, text_w = self._draw_card_and_thumb(painter, rect, is_selected, is_hover, thumb_pixmap)
        if text_w < 60:
            painter.restore()
            return
        ...
        date_w = self._fm_title.horizontalAdvance(date_text)
...
class PinItemDelegate(BaseCardDelegate):
    """卡片式贴图条目代理：复用 BaseCardDelegate 的尺寸规格。"""
    ...
    def paint(self, painter: QPainter, option, index) -> None:
        ...
        text_x, text_w = self._draw_card_and_thumb(painter, rect, is_selected, is_hover, thumb_pixmap, "无预览")
        if text_w < 60:
            painter.restore()
            return
