...
class OverlayToolSmokeTest(unittest.TestCase):
    ...
    def test_select_mode_masks_low_contrast_translucent_edge_lines(self) -> None:
        _ensure_app()
        cfg = Config()
        cfg.snap_to_windows = False
        raw = QPixmap(800, 600)
        ...
        painter = QPainter(raw)
        try:
            painter.fillRect(QRect(0, 109, 800, 2), QColor(96, 96, 96))
            painter.fillRect(QRect(0, 390, 800, 2), QColor(96, 96, 96))
        finally:
            painter.end()
        display = raw.copy()
        overlay = FloatingSnipOverlay(
        ...
        outside_xs = (40, rect.left() - 10, rect.right() + 10, overlay.width() - 40)
        for y in (rect.top() + 9, rect.top() + 10, rect.bottom() - 9, rect.bottom() - 8):
            with self.subTest(outside_y=y):
                for x in outside_xs:
                    color = canvas.pixelColor(x, y)
                    self.assertLess(color.red(), 50)
                    self.assertLess(color.green(), 50)
                    self.assertLess(color.blue(), 50)
            with self.subTest(interior_y=y):
                self.assertEqual(canvas.pixelColor(rect.center().x(), y).getRgb()[:3], (96, 96, 96))

    def test_select_mode_masks_horizontal_lines_in_side_dim_bands(self) -> None:
        _ensure_app()
        cfg = Config()
        cfg.snap_to_windows = False
        raw = QPixmap(800, 600)
        ...
        painter = QPainter(raw)
        try:
            painter.fillRect(QRect(0, 250, 800, 2), QColor(96, 96, 96))
        finally:
            painter.end()
        display = raw.copy()
        overlay = FloatingSnipOverlay(
        ...
        for y in (250, 251):
            with self.subTest(side_band_y=y):
